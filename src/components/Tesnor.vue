<template>
  <textarea v-model.passage="passage" cols='80' rows='30' type="text" />
  <br>
  <input type="text" v-model.question='question'>
  <br>
      <button @click="runModel()">Ask AI</button>
      <br>
      <p>{{answer.data.text}}</p>
      <!-- <div v-for="a in answer.data" :key="a">
        {{a.text}}
      </div> -->
</template>
...
export default {
  name: 'Tensor',
  setup() {
    const answer= reactive({data:[]})
    const question= ref('Where was telsa born?')
    const passage = ref(`Nikola Tesla (/ˈtɛslə/;[2] Serbo-Croatian: [nǐkola têsla]; Serbian Cyrillic: Никола Тесла;[a] 10
